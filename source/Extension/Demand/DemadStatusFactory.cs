...
{
    public class DemandFlowFactory
    {
        public static Dictionary<DemandStatusEnum, Func<DemandFlowEntity, (bool, string)>> dicDemandFlow = new Dictionary<DemandStatusEnum, Func<DemandFlowEntity, (bool, string)>>();
        private (bool, string) _updateRes = (false, "");

        public DemandFlowFactory()
        {
            dicDemandFlow.Add(DemandStatusEnum.Rejected, DoRejectFlow);
            dicDemandFlow.Add(DemandStatusEnum.Pending, DoPendingFlow);
            dicDemandFlow.Add(DemandStatusEnum.Processing, DoProcessingFlow);
            dicDemandFlow.Add(DemandStatusEnum.Verify, DoVerifyFlow);
            dicDemandFlow.Add(DemandStatusEnum.Completed, DoCompletedFlow);
        }
        ...
        public (bool, string) DoRejectFlow(DemandFlowEntity flowDataEntity)
        {
            ...
            if (_updateRes.Item1)
            {
                ...
                flowDataEntity.MailService.Send(new MailEntity
                {
                   ...
                   $"需求單連結：<a href='http://10.54.215.210/MOD4/Demand/Edit?sn={flowDataEntity.OldDemandOrder.orderSn}&orderId={flowDataEntity.OldDemandOrder.orderNo}' target='_blank'>" + flowDataEntity.OldDemandOrder.orderNo + "</a>" +
                    " <br /><br />謝謝"
                });
            }
